...
// GET /api/public-area-maintenance/today?branch_id=xxx&date=2026-07-27
// 今日公區：早上/下午日常 + 隊長任務 + 符合本週的每週輪替項目
router.get('/today', async (req, res) => {
  const { branch_id, date } = req.query;
  const wom = weekOfMonth(date);
  const weeklyCycle = wom % 2 === 1 ? 'weekly_odd' : 'weekly_even'; // 1,3,5週->odd／2,4週->even
  ...
  const { data: completions } = await supabase
    ...
    .select('template_id, status, completed_by, completed_at, staff:completed_by(name)')
    .eq('branch_id', branch_id)
    .eq('period_key', date);
  ...
  res.json((templates ?? []).map((t) => ({ ...t, completion: compMap[t.id] || null })));
});

// GET /api/public-area-maintenance/month?branch_id=xxx&month=2026-07
// 本月/本季公區：月清 + 符合當季的季清 + 符合當月奇偶的雙月清
router.get('/month', async (req, res) => {
  const { branch_id, month } = req.query; // month 格式 'YYYY-MM'
  ...
  res.json(applicable.map((t) => ({ ...t, completion: compMap[t.id] || null })));
});
