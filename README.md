# 路得行旅 每日營運系統 — 後端 API

目前包含三組核心功能（對應房務打掃 → 客務C班巡房檢查 → 業績獎金試算 這條流程）：

- `POST /api/login` — 同仁用登入代碼登入
- `POST /api/room-cleanings/assign` — 小隊長分配房號給房務同仁
- `GET  /api/room-cleanings/mine` — 房務同仁：今日負責房號清單
- `POST /api/room-cleanings/:id/complete` — 標記房間完成
- `GET  /api/room-cleanings/pending-inspection` — 客務C班：待檢查房號
- `POST /api/room-cleanings/:id/inspect` — 巡房檢查結果（正常 / 回報缺失）
- `GET  /api/bonus/daily` — 當日房務業績獎金試算

## 部署到 Railway（step by step）

1. **建立 Supabase 專案**（如果還沒做）
   - [supabase.com](https://supabase.com) → New Project
   - SQL Editor 貼上 `schema_v2.sql` 全部內容並執行
   - Settings → API，複製 `Project URL` 和 `service_role` key

2. **把這個資料夾放到 GitHub**
   - 在 GitHub 開一個新 repo（例如 `norden-daily-ops-backend`）
   - 把這整個資料夾的內容 push 上去（或用 GitHub 網頁介面直接上傳檔案）

3. **在 Railway 建立服務**
   - Railway 專案 → New → Deploy from GitHub repo → 選剛剛那個 repo
   - Variables 分頁，新增兩個環境變數：
     - `SUPABASE_URL` = 剛剛複製的 Project URL
     - `SUPABASE_SERVICE_ROLE_KEY` = 剛剛複製的 service_role key
   - Railway 會自動偵測 `package.json` 並跑 `npm start`

4. **確認部署成功**
   - 打開 Railway 給的網址（例如 `xxx.up.railway.app`），應該會看到 `{"status":"ok",...}`

## 本機測試（可選）

```bash
npm install
cp .env.example .env   # 填入 Supabase 資訊
npm run dev
```

## 同仁端網頁

`public/` 資料夾裡是三個網頁，跟後端部署在同一個 Railway 服務，不用另外設定網址：

- `index.html` — 登入頁（輸入登入代碼，依職務自動導向對應頁面）
- `checklist.html` — 房務同仁：今日房號打卡
- `inspect.html` — 客務C班：巡房檢查、回報缺失（含拍照）

部署後打開 Railway 給的網址就會直接看到登入頁。

新增：

- `assign.html` — 小隊長：今日房號分配（下拉選單指派每間房給誰，一次儲存）
  - 可從 checklist.html 上方連結進入
  - 對應 API：`GET /api/rooms`、`GET /api/staff/list`、
    `GET /api/room-cleanings/assignments`、`POST /api/room-cleanings/assign-batch`

- `dashboard.html` — 店經理：電腦版營運總覽（今日房況、房務業績獎金總表、未處理缺失、
  本月細清進度）
  - 對應 API：`GET /api/dashboard/summary`
  - 登入時角色若為「店經理」會自動導向這頁

- `deep-clean.html` — 樓主：本月細清排程（設定各樓層樓主、各樓層排第幾週、產生本月
  任務、標記完成）
  - 對應 API：`GET/POST /api/deep-clean/floor-owners`、`GET /api/deep-clean/templates`、
    `GET /api/deep-clean/month`、`POST /api/deep-clean/generate`、
    `POST /api/deep-clean/:id/complete`
  - 可從 checklist.html 上方連結進入
- `shift.html` — 客務A/B/C班：今日班別任務打卡（依 daily/weekday/monthly_date 規則
  自動判斷今天該出現哪些任務）
  - 對應 API：`GET /api/shift-tasks/today`、`POST /api/shift-tasks/:id/complete`
  - 登入時角色為「客務A班」「客務B班」會直接導向這頁；「客務C班」導向 inspect.html，
    但頁面上方有連結可以切換過去

- `handover.html` — 客務：交班表（對應紙本格式的結構化欄位＋可自訂欄位）
  - 「公區、客房設備異常回報」欄位會自動帶入當天所有任務按「回報異常」產生的內容，
    同仁可以再編輯或補充
  - 對應 API：`GET /api/handover/today`、`GET /api/handover/defects-today`、`POST /api/handover`
  - 同一天同一班別重複送出會更新，不會重複建立
- `shift.html` 每個任務旁邊多了「回報異常」按鈕（不管任務有沒有標記完成都能按），
  對應 API：`POST /api/issues/report`——這支是通用的，之後房務/細清任務要加同樣的
  回報按鈕，可以直接呼叫同一支 API

**部署前多一步**：先到 Supabase SQL Editor 執行 `schema_v3_handover.sql`（新增交班表
的資料表），再上傳程式碼。

- `schedule.html` — 店經理：排班表（取代 Excel）
  - 依部門分組顯示同仁，每格是下拉選單選當天班別代碼
  - 自動統計每人休假天數（跟目標天數不符會標紅）、每日客務/房務上班人數（低於門檻標紅）
  - 點日期標題可切換「禁休日」（粉色），跟休假天數目標、最低上班人數都可以每月調整
  - 對應 API：`GET/POST /api/schedule/*`
  - **這是協助計算的工具，不會自動生成班表**——班別代碼要主管自己填，系統只負責統計跟警告
- `templates.html` — 店經理：客務班別任務範本管理（新增/編輯/停用任務項目，不用等改程式碼）
  - 對應 API：`GET/POST/PUT /api/templates/shift-tasks*`
- `offday.html` — 排休日登入會看到的簡單頁面

**登入流程改了**：現在登入時會先查今天的排班表（`staff_schedule`），如果today有排班紀錄，
依實際排的班別代碼導向對應頁面（A/B→shift.html，C→inspect.html，房→checklist.html，
1→offday.html）；如果今天沒有排班紀錄（例如還沒排班、或代碼是中/特/管/PT這種還沒對應頁面的），
才會退回用同仁的固定職務判斷，維持原本行為當備援。

**部署前多一步**：先到 Supabase SQL Editor 執行 `schema_v4_schedule.sql`。

- `issues.html` — 店經理：異常處理（取代「只能在對話裡看」的缺失回報）
  - **處理清單**：所有來源（客房/客務任務/公共空間/細清）的異常，可篩選待處理/已處理，
    有「標記已處理」按鈕
  - **房間重複問題**：依房號統計一段期間內（14/30/90天可切換）的異常次數，3次以上會
    特別標紅，方便看出「同一間房反覆出問題」
  - 對應 API：`GET /api/issues/list`、`POST /api/issues/:id/resolve`、
    `GET /api/issues/room-trends`
  - 可從店經理儀表板上方連結進入

- **系統偵測異常（自動）**：同一間房在30天內累積到3次異常回報（不管有沒有處理過都算），
  系統會自動在 `anomaly_logs` 產生一筆警示，店經理登入儀表板會直接看到紅色提示區塊，
  不用自己去異常處理頁翻。已經有一筆待處理的同類型警示不會重複產生，避免洗版。
  店經理按「確認處理」後，下一次再累積到門檻才會產生新的一筆。
  - 對應 API：`GET /api/issues/anomalies`、`POST /api/issues/anomalies/:id/resolve`
  - 偵測邏輯寫在 `POST /api/room-cleanings/:id/inspect` 裡，目前門檻（3次/30天）是寫死在
    程式碼裡，之後如果想讓店經理自己調整門檻，可以再加設定頁面

- `manager-checklist.html` — 店經理：每日巡館清單（對應原始文件的八項工作：每日巡館、
  住房率確認、人力配置確認、客訴與旅客反饋、設備維修案件、房務及櫃檯品質抽查、
  同仁教育與溝通、異常事件回報）
  - 「住房率確認」「人力配置確認」會自動帶入系統既有數字（今日房務完成數、今日排班
    人數），不用手動再查一次
  - 對應 API：`GET /api/manager-checklist/today`、`POST /api/manager-checklist/:id/complete`
  - 可從店經理儀表板上方連結進入

**部署前多一步**：先到 Supabase SQL Editor 執行 `schema_v5_manager_checklist.sql`。

- `learning-map.html` — 新人教育訓練（依「房務員工訓練紀錄表」「新人學習確認表」兩份
  實際文件內容建置，房務57項、客務48項）
  - 選職務類別（房務/客務）→ 選同仁 → 看到完整學習清單，依分類（房務）或主題+分類
    （客務）分組
  - 每一項可以填培訓員、教學日期、驗收結果（合格/不合格），對應原本 Excel 的欄位
  - 上方有整體進度（已合格幾項/總項數）
  - 房務類別多一塊「整房實作考核」，可以新增考核紀錄（房間組成、限時、分數、合格與否）
  - 對應 API：`GET/POST /api/training/progress`、`GET/POST /api/training/exams`
  - 目前沒有限制誰能填寫/編輯（跟你之前說小隊長不用權限控管一致），任何登入的同仁都
    可以幫任何人填寫進度，之後如果想限制只有教練/主管能編輯可以再討論

**部署前要跑兩份 SQL，依序執行**：
1. `schema_v6_training.sql`（建表 + 建立兩條學習地圖）
2. `schema_v6_training_seed.sql`（把57+48項訓練內容塞進去，要在第一份執行成功後才能跑）

- **四階段考核**（認識→操作→獨立→穩定）已經整合進 `learning-map.html` 同一頁：
  - 選好同仁後，下方會出現四階段的橫向進度條，點哪個階段就編輯哪個階段
  - 每個階段都有十個面向評分（工作態度、專業能力、工作品質、工作效率、溝通能力、
    團隊合作、主動性、責任感、問題處理、品牌認同），對應原始文件的考核內容
  - 填評核者、備註、通過/尚未通過，儲存後階段條會變色標示已通過
  - 對應 API：`GET/POST /api/assessment/stages`
  - 四個階段目前**沒有強制循序**（可以直接跳去填「穩定」而不用先過「認識」），
    如果你希望系統強制照順序才能往下一階段，跟我說一聲再加邏輯

**部署前多一步**：先到 Supabase SQL Editor 執行 `schema_v7_assessment.sql`。

## 職務簡化

`房務小隊長`／`房務樓主` 這兩個職務已經拿掉，統一併回 `房務人員`。原因：
- 小隊長本來就是每天輪值，職務欄位標記反而不準確
- 樓主的認養狀況已經有 `floor_owners` 表在追蹤（誰在哪個月負責哪個樓層）
- 房號分配（`assign.html`）、細清排程（`deep-clean.html`）本來就沒有限制只有小隊長／樓主能用，
  任何房務人員登入都看得到那兩個連結，拿掉職務不影響任何現有功能

## 總公司模組

- `hq-dashboard.html` — 總公司：跨館總覽 + 任務分派
  - 每個館別一張卡片：今日房務完成度、未處理異常（含最久幾天沒處理）、未處理缺失、
    店經理巡館完成度、待處理總公司任務數
  - 積壓超過7天沒處理的異常或缺失，卡片會整個標紅提醒
  - **點卡片可以展開實際內容**：系統偵測異常的完整描述、未處理缺失清單（含房號/來源、
    回報人、時間），也能直接在這裡標記已處理，不用切到各館分別查
  - 下方可以直接交辦新任務給指定館別（標題、說明、期限、交辦人）
  - 對應 API：`GET /api/hq/overview`、`GET/POST /api/hq/tasks`、
    `GET /api/issues/anomalies`、`GET /api/issues/list`（沿用既有的異常處理 API，
    差別只是總公司可以指定任何館別的 branch_id 查）
  - 登入時角色若為「總公司」會自動導向這頁
- 店經理儀表板新增「📋 總公司交辦事項」區塊，會顯示自己館別待處理的任務，可以填回報內容
  後標記完成
  - 對應 API：`GET /api/hq/tasks/branch`、`POST /api/hq/tasks/:id/complete`

**部署前要跑 SQL**：執行 `schema_v8_hq.sql`。**這份會做資料異動**（把房務小隊長／樓主的同仁
改成房務人員、刪除這兩個職務），建議先在 Supabase 確認目前有沒有同仁掛在這兩個職務下，
執行後這些同仁的職務欄位會自動變成「房務人員」，不會影響他們的登入代碼或其他資料。

## 台東1館 + 分館獎金規則

- 新增台東1館館別，房號63間（A-E責任區，房號含2/3樓一般房與R樓/RF頂樓房型）
- `rooms.zone` 新欄位：台東1館用來記錄A-E責任區，台中館沒有這層留空即可
- **獎金規則改成每個館可以不一樣**（`bonus_settings` 新增 `rate_type`、`tier1_max`、
  `tier1_rate`、`tier2_rate`、`require_full_completion` 欄位）：
  - 台中館：`linear`，淨間數 × 固定單價（原本邏輯不變）
  - 台東1館：`tiered`，前12間$10/間、第13間起$20/間，且 `require_full_completion=true`
    ——當天沒有在期限前把被分配的房間全部完成，整天不算獎金
  - 計算邏輯統一寫在 `src/lib/bonusCalc.js`，`bonus.js` 跟 `dashboard.js` 都呼叫同一支，
    之後台東2館如果規則又不同，只要調整 `bonus_settings` 的資料，不用改程式碼
  - 儀表板的獎金表格，如果當天因為「未全部完成」被取消資格，會顯示提示文字

**部署前要跑 SQL**：執行 `schema_v9_tt1.sql`。

**還沒做的部分（照你選的優先順序，下一步繼續）**：房號分區保養排程（A-E區週循環）、
公區保養月曆（日常+月清+雙月清）、每日房務日誌的小隊編組結構。

## 這次修正（依台東1館店長確認的實際規則）

- **獎金門檻改成固定數字，不是「全部完成」**：15:00前完成間數要達到門檻（台東1館是12間）
  才有獎金，不是「把當天分配的房間全部做完」——如果分配15間、15:00前完成13間，一樣有獎金
  （淨間數=13扣缺失），只要13≥12門檻就過關
  - 對應欄位：`bonus_settings.min_rooms_for_bonus`
- **PT（兼職）同仁不計入房務打掃獎金**：`staff` 表新增 `is_part_time` 欄位，獎金計算的
  同仁清單會排除這些人
- **排班表新增「房務小隊長」列**：經理排班時可以直接在排班表指定每天的房務小隊長，
  跟班別代碼分開存（`daily_team_leads` 表），對應 `GET/POST /api/schedule/*` 一併回傳/儲存
- **B班在需要支援巡房時，`shift.html` 也會顯示「前往巡房檢查」連結**（原本只有C班看得到）

**部署前要跑 SQL**：執行 `schema_v10_fixes.sql`。

## 這次修正（獎金確認關卡 + 動態班別任務）

- **獎金只算客務已經巡房確認過的房間**：房務同仁標記「完成」後，如果客務還沒去巡房
  檢查（`checked_by` 還是空的），這間房不會算進獎金總表，也不會出現在店經理儀表板的
  獎金數字裡，等客務確認過（不管正常還是回報缺失）才會被算進去
- **班別任務清單改成「排班型態切換」，不是自動合併**：系統查當天有沒有排C班，有排就用
  「ABC」版本的任務清單，沒排（例如AABB）就用「AABB」版本——這兩套內容是台東1館提供的
  真實日誌逐條建進去的，AABB版本裡C班的工作已經按時段拆分揉進A、B班（有些用詞跟時間點
  也不一樣，不是系統自動硬塞），如果某個班別沒有對應的AABB版本會自動退回ABC版本
  （台中館目前只有ABC版本，不受影響）
- **修正房號分配的資料覆蓋 bug**：小隊長追加分配新房號時，已經完成的房間不會再被
  洗回「待完成」

**部署前要跑兩份 SQL，依序執行**：`schema_v11_shift_pattern.sql`（先加欄位）→
`schema_v11_seed.sql`（再塞入台東1館ABC/AABB兩套完整任務內容，共136項）。

## 登入備援機制調整

客務（A/B/C班）現在**一定要有排班表紀錄**才能看到今日任務——沒有排班紀錄的客務同仁登入
會看到 `unscheduled.html` 提醒頁（「今天還沒有排班紀錄，請聯繫店經理確認本月排班表」），
不會再退回用固定職務猜測顯示哪一版任務清單。

房務、店經理、總公司這幾個角色**維持原本的固定職務備援**，不受影響：
- 房務的房號是小隊長另外分配的，跟排班表是兩件事
- 店經理、總公司本來就不是靠排班表運作的角色

## 客務職務簡化

`客務A班`／`客務B班`／`客務C班` 這三個職務拿掉了，統一併回 `客務人員`。理由跟房務
小隊長／樓主一樣：沒有人是固定哪一班，每天是誰、上哪一班，是排班表決定的，職務欄位
不該綁死。系統原本判斷「今天顯示哪一版任務清單」，本來就是看排班表的 `shift_code`
（A/B/C），跟固定職務名稱無關，所以拿掉這三個職務不影響任何現有功能，只是讓資料更準確。

**部署前要跑 SQL**：執行 `schema_v12_frontdesk_role.sql`（會把掛在A/B/C班職務下的同仁
自動改成客務人員，再刪掉這三個職務，不影響其他資料）。

## 台東1館：A-E責任區保養排程

跟台中館的「樓主月排一輪」邏輯完全不同——是「每個責任區的每間房，每週固定做什麼」，
所以另外做一個新頁面 `zone-maintenance.html`，不影響台中館原本的 `deep-clean.html`。

- 選責任區（A-E）→ 看到該區所有房間 x 這個月適用任務的表格（房號當欄、任務當列，
  跟你給的紙本表格排版一致）
- 任務分「第一週~第四週」（固定循環，全部責任區同一時間做同一週的項目，不需要輪流）、
  「月任務」（天花板/牆壁/層板檢查、上油漆、窗簾保養、曬被曬枕）、「季清」
  （依月份自動判斷該季適用哪些項目：1/4/7/10月是翻床+床底棉絮，2/5/8/11月是水盤+
  出風口，3/6/9/12月是主機濾網+廁所濾網）
- 點格子直接標記完成（暫時沒做取消勾選，避免誤觸）
- 責任區負責人可以直接指定（暫不輪替，符合你說的「經理直接指定」）
- **只有店經理能切換責任區、能指派負責人**：房務同仁登入這頁，只會看到自己被指派的
  那一區，沒有切換選項也看不到指派控制項；還沒被指派的人會看到提醒文字。這個限制是
  畫面層級的（不是後端強制擋），跟系統其他地方的權限設計一致
- 下方有「油漆巡視狀態」表，每間房可以記錄目前狀態（乾淨/需油漆/壁癌）跟備註，
  這是持續性狀態，不是每月重新歸零
- 對應 API：`GET/POST /api/room-maintenance/*`
- 打卡頁上方的保養排程連結會**自動判斷這個館是用樓層制還是責任區制**，顯示對應版本，
  不會再讓台東1館同仁看到台中館的樓層版連結

**部署前要跑兩份 SQL，依序執行**：`schema_v13_zone_maintenance.sql` → `schema_v13_seed.sql`。

## 台東1館：公區保養月曆

`public-area.html`，從房務打卡頁點「公區保養 →」進去，分兩個分頁：

- **今日公區**：早上日常公區、下午日常公區、隊長任務，還有依「這週是第幾週」自動判斷
  該顯示哪一組每週輪替項目（清蜘蛛網是1、3週；盆栽擦拭是2、4週）
- **本月／本季公區**：月清（22項，每月都要做）+ 依月份自動判斷適用的季清（三組月份各兩三項）
  + 依當月奇偶自動判斷適用的雙月清（椅墊清潔／懶人肩頭保養）
- 對應 API：`GET /api/public-area-maintenance/today`、`GET /api/public-area-maintenance/month`、
  `POST /api/public-area-maintenance/:id/complete`

**加了小隊長分配機制**：公區任務現在也要小隊長先分配才會出現在同仁的打卡頁，跟房號分配
邏輯一致：
- `public-area-assign.html`（從打卡頁點「小隊長：公區任務分配」進去）：分「今日公區」「本月／本季公區」
  兩個分頁，每項任務用下拉選單選要分配給誰，點「儲存分配」
- 同仁的 `public-area.html` 現在只會顯示**分配給自己**的項目，沒被分配到會看到
  「目前還沒有分配到任何項目，請跟小隊長確認」
- 已經標記完成的項目，追加分配時不會被覆蓋（跟房號分配的修正邏輯一致）
- 對應 API：`POST /api/public-area-maintenance/assign-batch`

**部署前要跑三份 SQL，依序執行**：`schema_v14_public_area.sql` → `schema_v14_seed.sql` →
`schema_v15_pa_assign.sql`（如果 v14 兩份已經跑過，只需要補跑這份新的）。

**已知小缺口**：這個連結目前台中館的房務同仁登入也看得到（會顯示空白清單，因為台中館
沒有建這套內容），沒有像責任區保養排程那樣做館別判斷隱藏，之後如果想補上可以再說。

## 小隊長權限跟排班表接起來

之前「今日房號分配」「公區任務分配」這兩個連結是任何房務同仁都看得到，沒有真的去檢查
排班表上今天指定的小隊長是誰。現在改成：**只有今天在排班表被指定為小隊長的那位同仁，
才會在打卡頁看到這兩個連結**，其他房務同仁看不到（畫面上隱藏，不是後端強制擋，跟系統
其他地方的權限設計一致）。

前提是店經理有在排班表把「今日房務小隊長」那一列填好，如果沒填，當天就不會有人看到
這兩個連結——這也是提醒排班表要確實填寫的一個附帶效果。

對應 API：`GET /api/schedule/team-lead-today`

## 房號分配預帶責任區負責人

確認過台東1館的實際邏輯：每日打卡分工跟週保養責任區其實是**同一套人馬**，小隊長分配
房號時會優先把房間分給該房間所屬責任區的負責人，這樣掃房跟保養可以一次做完。

`assign.html`（房號分配頁）現在會：
- 顯示每間房所屬的責任區跟該區負責人（例如「201（A區・友宜）」），方便小隊長判斷
- **不會自動選人**，下拉選單維持「未分配」，小隊長還是要自己看著標籤手動判斷、平均分配，
  系統只提供資訊參考，不做決定
- 台中館的房間沒有責任區資料，不受影響，行為維持原樣

## 客訴事後補登機制

客務接到客訴時（通常是事後才知道），可以回頭登記，客訴會算成缺失，影響那天房務同仁的獎金：

- `complaint-register.html`（從客務任務頁點「客訴登記 →」進去，任何客務同仁都能用，
  不限特定班別，符合你說的「C班或B班都可能去登記」）
- 選發生日期、房號、填客訴內容、可以附照片
- 系統會找出那間房那一天的打掃紀錄，標記成缺失（如果原本已經有其他缺失備註，會合併
  顯示，不會覆蓋掉）——這樣一來，`/api/bonus/daily` 重新計算時就會自動把這筆客訴算進去，
  不用手動調整獎金
- 同時會寫入統一的 `defect_logs`，所以也會出現在店經理的「異常處理」頁，也會被「同一間房
  反覆出問題」的自動偵測納入計算
- 如果那天那間房根本沒有打掃紀錄（例如沒被分配到），系統會建立一筆沒有指定房務同仁的
  紀錄純粹留存客訴內容，不會誤算到任何人的獎金
- 對應 API：`POST /api/room-cleanings/register-complaint`

## 每月工作夥伴自評表

依你提供的四份自檢表（主管職／房務部／房務部中印／客務部）建置，共43題（共同11題、
主管職11題、房務10題、客務11題），房務題目含中印雙語（照你原本文件的印尼文翻譯）。

**時程**（2026-09 更新，全部以**台灣時間**計算）：

| 日期 | 事項 |
|---|---|
| 每月 1 號 | 系統發布「上個月」的自評表（例如 9/1 發布 8 月的表） |
| 每月 10 號前 | 同仁完成填寫並送出 |
| 每月 20 號前 | 主管完成面談（審閱頁按「完成面談」） |

逾期只會標示提醒，不會擋住填寫或面談。時程設定在 `src/routes/selfEval.js` 最上面的 `SCHEDULE`，
要改日期只要改那裡。Railway 伺服器跑在 UTC，所以日期一律先換算成台灣時間（不然台灣時間 1 號
早上 8 點前還會被當成上個月），前端也改用伺服器回傳的 `today` 判斷逾期，不用瀏覽器自己的 UTC 日期。

- `self-eval.html`（同仁填寫）
  - 系統自動依同仁職務帶出「共同題目＋對應部門專屬題目」
  - 每題勾是/否＋可填說明，勾「否」不會扣分——勾否是跟主管面談、討論怎麼改善的起點
  - 可以先「儲存草稿」，之後再回來改，送出後鎖定不能再改
  - 上方顯示 10 號填寫期限與 20 號面談期限，超過 10 號沒送出會顯示逾期提醒
  - 對應 API：`GET /api/self-eval/form`、`POST /api/self-eval/save`

- `self-eval-review.html`（店經理面談）
  - 只看得到**自己館別**的同仁清單，上方顯示本期期限與「已送出 x / N・已完成面談 y / N」
  - 狀態：尚未填寫／填寫中（草稿）／待面談／已完成面談；超過期限會標「逾期未送出」「面談逾期」
  - **同仁送出之後才能點進去面談**（草稿不能被主管鎖定，後端也會擋）
  - 逐題填「主管確認」是/否＋說明、面談紀錄、面談日期（預設今天）
  - 「完成面談」後這份表就鎖定，不能再改
  - 對應 API：`GET /api/self-eval/schedule`、`GET /api/self-eval/branch-overview`、
    `GET /api/self-eval/submission-detail`、`POST /api/self-eval/manager-review`

- `self-eval-templates.html`（總公司管理題目，總公司總覽頁有連結）
  - 可以新增、編輯、停用（不是真的刪除，保留歷史紀錄關聯）每一題，含分類跟印尼文翻譯
  - 對應 API：`GET/POST/PUT /api/self-eval/templates*`

**權限說明**：三層權限（總公司改題目／店經理審閱自己館別／同仁只能寫自己的）都是**畫面
層級**做的——店經理審閱頁本來就只查自己 `branch_id` 的資料，同仁填寫頁本來就只看得到
自己的表，題目管理頁只有連結掛在總公司總覽頁——但跟系統其他地方一樣，沒有做到後端強制
驗證身分這種程度，是延續整套系統目前的權限設計方式。

**部署前要跑兩份 SQL，依序執行**：`schema_v16_self_eval.sql` → `schema_v16_seed.sql`。

**新增：還沒填會提醒**——`self-eval-reminder.js` 這個共用元件會在房務打卡頁、客務任務頁、
巡房檢查頁、店經理儀表板、總公司總覽這幾個大家平常會打開的頁面上方，檢查「上個月的自評表
送出了沒」，沒送出會顯示黃色提醒 banner（超過10號沒送出會變紅色標記「已逾期」），點連結
可以直接跳去填寫頁。已經送出的人不會看到這個提醒。

**總公司：各館自評進度**（`self-eval-hq-flagged.html`，2026-09 由原本的「自評異常彙總」改版，
檔名沿用所以舊連結不會壞）
- 原本是列出勾「否」的項目，但**勾否不代表異常**，自評表的目的就是勾否之後主管跟同仁面談改善，
  所以改成只看**完成進度**，不顯示作答內容（原本的 `GET /api/self-eval/hq-flagged` 已移除）
- 先選評核月份（從最早有人填的那一期到本期），畫面上方顯示本期的發布日、填寫截止、面談截止
- 全公司合計＋每個館一張卡片，分兩個階段顯示「已完成／未完成」人數：
  **同仁填寫**（10 號前送出）、**主管面談**（20 號前完成），超過期限還沒完成的會標紅「逾期」
- 點館別卡片可以看每位同仁的狀態：已送出（日期、是否超過期限才送出）／填寫中／尚未填寫，
  以及已完成面談（面談日期）／待面談／等同仁送出，可篩選「未完成填寫」「未完成面談」
- 應填名單＝目前在職同仁，加上那一期有填過但之後停用的同仁（看歷史月份才不會少算）；
  館別以填寫當時存下來的為準
- 對應 API：`GET /api/self-eval/hq-progress?eval_month=2026-08-01`

## 每日自評／每週目標確認

跟「每月自評表」不同——這個是輕量的日常反思，不是是非題稽核。

- **每日自評**（`daily-reflection.html`，打卡頁/任務頁有連結）：下班前填5題簡答（今天完成
  哪些工作、做得最好的、遇到的問題、需要的協助、明天要改善什麼），同一天可以重填覆蓋。
  店經理端 `daily-reflection-review.html` 可以看當天所有同仁填的內容。
- **每週目標確認**（`weekly-goals.html`）：同仁填本週目標、品質要求、完成/未完成事項、
  需要協助、下週重點，可以隨時修改直到主管確認。店經理端 `weekly-goals-review.html`
  列出本館所有同仁本週的填寫狀態，可以按「確認本週目標」鎖定（確認後同仁還是能看，
  但畫面上會標示已確認）。
- 對應 API：`GET/POST /api/reflections/daily*`、`GET/POST /api/reflections/weekly*`

**部署前要跑 SQL**：執行 `schema_v17_daily_weekly.sql`。

## 台東1館：學習地圖 + 修正分館 bug

**修正的 bug**：學習地圖原本沒有依館別篩選，如果兩館都建學習地圖內容會互相干擾（查詢
邏輯只看職務類別，不看是哪一館）。現在 `GET /api/training/progress` 一定要帶 `branch_id`，
台中館跟台東1館的學習地圖完全分開，不會互相看到。

**台東1館的學習地圖內容**（照你這次給的「房務正職學習單」「客務學習單」建置）：
- **房務**：70項，分「保養」（跟責任區保養排程的週期任務內容一致，含印尼文翻譯，因為
  台東1館有印尼籍實習生）跟「公區」（跟公區保養月曆的日常項目一致）
- **客務**：40項，照「學習大綱」建置，分「基本概念」（房型/價格/接單/取消政策/特殊訂單）
  跟「業務內容」（A/B/C班各自業務＋其他雜項）跟「儲物空間」
- `learning_units` 新增 `item_name_id` 欄位存印尼文翻譯，畫面上會在中文項目下方用小字
  顯示

**部署前要跑兩份 SQL，依序執行**：`schema_v18_tt1_learning.sql` → `schema_v18_seed.sql`。

## 四個新功能（NO SHOW／評論回報／例行事項／開班收班日報）

- **NO SHOW記錄**（`no-show.html`，客務任務頁有連結）：客務發現No Show就登記一筆，姓名/訂單、
  房型、有沒有收費、金額、備註，當天清單即時顯示
  - 對應 API：`GET/POST /api/no-show`

- **評論回報**（`review-check.html`）：固定四平台（BK/AG/Ctrip/Google），每天巡查一次，
  正常就按「已檢查」，發現故障/客訴類評論就按「發現異常評論」填說明
  - 對應 API：`GET /api/review-checks/today`、`POST /api/review-checks/check`

- **例行事項總表**（採購/銀行/總務）：
  - 同仁端 `routine-tasks.html`：只看得到分配給自己的項目，可以更新進度說明跟狀態
  - 店經理端 `routine-tasks-manage.html`：看得到全部項目，可以新增、編輯、指派負責人
  - 這是**持續存在的清單**，不是每日/每月重新產生，經理每月要換負責人直接改指派欄位即可
  - 對應 API：`GET/POST/PUT /api/routine-tasks*`

- **開班收班日報**（`manager-daily-report.html`，只有店經理用）：跟台中館的巡館打勾清單不同，
  是敘述式日報——開班時填「預計執行事項」，收班時填「完成事項/未完成待追蹤/各項抽檢/
  總公司反應事項」，同一天算一份，先開班再收班
  - 對應 API：`GET /api/manager-reports/today`、`POST /api/manager-reports/open`、
    `POST /api/manager-reports/close`

**部署前要跑 SQL**：執行 `schema_v20_tt1_extras.sql`。

## 排班表 Excel 匯入

`schedule-import.html`（排班表頁面上方有連結），讓經理可以直接上傳他們原本習慣用的
Excel 班表檔案，系統自動解析寫入，不用逼他們改用新介面重新輸入一次。

**運作方式**：
- 上傳後系統會自動找出表格裡「1、2、3...」這樣連續遞增的日期橫列，抓出每天對應的欄位
- 用**姓名**去比對系統裡的同仁資料（同仁的 `staff.name` 要跟 Excel 裡的姓名完全一致才
  抓得到，多一個空格或打字不同都比對不到）
- 比對不到的姓名會列在警示區，不會被匯入，需要的話手動在排班表頁面補
- 解析完會先顯示**預覽表格**，確認內容沒問題才需要按「確認匯入」，不會你一上傳就直接
  寫進資料庫
- 匯入邏輯是呼叫既有的 `POST /api/schedule/save`，所以會沿用「已完成打卡的房間不會被
  覆蓋」這類保護機制（不過排班表本身沒有這層保護，重複匯入同一天會直接覆蓋原本的班別）
- 用瀏覽器端的 SheetJS 解析（不用另外裝軟體），跟你之前在其他系統看過的 Excel 匯入功能
  原理一樣

**已知限制**：
- 只讀取檔案裡的**第一個工作表**，如果班表分好幾個分頁（例如客務一頁、房務一頁），
  要分開匯入兩次
- 解析邏輯是抓「連續遞增數字」當日期列，如果表格格式差異很大（例如日期不是橫向排列），
  可能抓不到，會直接顯示錯誤訊息而不是匯入錯的資料

**這次依實際8月班表校正的地方**：
- 姓名比對改成「先完全比對，比對不到再檢查整列前面幾欄合併起來的文字裡有沒有包含
  姓名的中文部分」，這樣即使姓名沒填在正確欄位、跟到職日期或外文名混在一起（例如
  實習生那種格式：「115.4.23 實習生-因德拉 Wijaya, Indra Pratama」），也抓得到
- 自動偵測「房務小隊長」列，把裡面的暱稱（**姓名最後一字**，例如黃紹華→華）比對回
  同仁，一併匯入每日房務小隊長設定，不用另外手動填排班表那個欄位
- 「櫃台/房務實際人數」這類統計列不會再被誤判成「比對不到的姓名」出現在警示區裡
- **員編目前不拿來當比對依據**（實際班表裡的員編偶爾會跟員工名冊對不起來，以姓名
  比對為準，比較不會匯錯人）

## 客務日誌四合一（掃單／評論／NO SHOW／個人清潔配額）

依實際使用的 CSV 重新設計，跟原本做的版本不一樣：

- **掃單記錄＋評論回報＋NO SHOW 合併成一個頁面**（`daily-frontdesk-log.html`，客務任務頁
  點「客務每日日誌」進去）：三個都是「一天一筆」的日常記錄，原本要跳三個頁面，現在
  一次填完，各自獨立儲存（填一段存一段也可以，不用整頁都填完才能存）
  - **評論回報**改成四平台各自填當天看到的內容（不是打勾），不確定就寫「無」
  - **NO SHOW**改成一天一筆，沒有發生就寫「無」，不是「有事件才新增」的清單
  - **掃單記錄**是全新功能：今日住房訂單數/異常、昨日新單數/遺漏，下面還有「互相提醒／
    案例分享」留言板
  - 對應 API：`GET/POST /api/order-sweep/*`、`GET/POST /api/review-checks/today`、
    `GET/POST /api/no-show/today`

- **個人清潔配額**（`staff-cleaning.html`，獨立頁面，因為性質是月度配額不是每日）：
  20項清潔項目分A-D四類，客務同仁每月自己挑不同類別完成4項，可以拍照佐證，畫面上方
  顯示自己本月進度（完成幾項、涵蓋幾個類別）
  - 對應 API：`GET /api/staff-cleaning/list`、`GET /api/staff-cleaning/my-progress`、
    `POST /api/staff-cleaning/:id/complete`

**已經不用的舊檔案**：`no-show.html`／`no-show.js`／`review-check.html`／`review-check.js`
這四個檔案功能已經被新版取代，打卡頁不會再連過去，留著也不會出錯（只是沒人連得到），
你可以之後有空再從 GitHub 刪掉，不刪也沒關係。

**部署前要跑兩份 SQL，依序執行**：`schema_v22_daily_logs.sql` → `schema_v22_seed.sql`。

## 例行事項 Excel/CSV 匯入

`routine-tasks-import.html`（從例行事項管理頁點「從 Excel/CSV 匯入」進去），支援上傳
.xlsx 或 .csv。

**只會解析「例行」表格區塊**（人員/項目/進度/預計完成日/實際完成日/NOTE 那種固定格式，
W1-W5週次項目），因為這個區塊格式規律；**上半段的「一次性任務」區塊格式太不規則**
（合併儲存格），不會被解析，那塊還是需要手動整理給我。

- 自動找出「人員」「項目」這樣的表頭列，用**負責人姓名**去比對系統同仁（先完全比對，
  比對不到再試試看姓名有沒有互相包含，例如「治為」可以比對到「王治為」）
- 有「實際完成日」欄位的項目會自動標記成「已完成」
- 一樣會先顯示**預覽表格**，確認沒問題才按「確認匯入」
- 比對不到負責人的項目，匯入後負責人會是空的，需要在例行事項管理頁面手動補上
- 對應 API：`POST /api/routine-tasks/bulk-import`

**注意**：這個匯入是**新增**，不會檢查重複——如果同一份項目匯入兩次，會變成兩筆一樣的
資料，需要自己在管理頁面刪除多的（目前管理頁面沒有做刪除功能，如果需要可以再加）。

## 學習地圖改成「店經理開啟才有」

原本任何同仁都會出現在學習地圖的選單裡，現在改成**預設沒有人有學習地圖**，要店經理
手動開啟才會出現：

- `staff` 表新增 `learning_map_enabled` 欄位，預設 `false`
- 店經理登入學習地圖頁面，選好職務類別後，畫面上方會多一塊「管理學習地圖開放名單」，
  列出該類別**全部**同仁，勾選就是開啟、取消勾選就是關閉
- 一般同仁登入，只會在下拉選單看到**已經被開啟**的人（通常是新人），不會看到店經理、
  也不會看到已經受訓完畢、不需要再顯示的資深同仁
- 對應 API：`GET /api/staff/list?learning_enabled=true`、
  `POST /api/staff/:id/learning-map-toggle`

**部署前要跑 SQL**：執行 `schema_v23_learning_toggle.sql`。

## 學習地圖改成指派制

原本的設計是「誰都能挑誰來改」，不合理——重新設計成三個角色各自用自己的帳號操作：

- **店經理**：`learning-assign.html`（儀表板點「指派培訓」）——選新人＋職務類別，看到
  全部學習項目，逐項指派要由誰教（下拉選單選教練），四階段考核跟整房實作考核也整合
  在這一頁一起評核
- **教練**：`my-teaching.html`（打卡頁/任務頁點「我的教學任務」）——用**自己的帳號**登入，
  看到分配給自己的教學任務（跨新人彙整），教完點「標記完成教學」＋填驗收結果，
  完成當下就會用自己的名字記錄培訓員跟教學日期，不用手動輸入
- **新人**：`learning-map.html`（打卡頁/任務頁點「我的學習地圖」）——現在是**唯讀**，
  只顯示自己的進度（哪些已合格、培訓員是誰、哪天教的），不能編輯，也看不到別人的資料

**已經棄用的舊機制**：之前「店經理開啟名單」那個勾選開關（`learning_map_enabled`）現在
不需要了，指派教學任務本身就會讓項目出現在新人的學習地圖裡，不用額外開關。這個欄位
留著沒關係，不影響功能。

對應 API：`GET/POST /api/training/assign-list`、`/assign`、`/my-teaching`、`/teaching-complete`

**部署前要跑 SQL**：執行 `schema_v24_learning_assign.sql`。

## 教學待辦提醒 + 學習地圖顯示條件 + 教學期限

- **教學任務改成主頁待辦提醒**，不是塞進今日任務（因為不一定要今天完成）：打卡頁、
  客務任務頁最上方，如果有分配給你的教學任務，會出現黃色（逾期則紅色）提醒 banner，
  顯示還有幾項、最近的期限，點連結直接跳去「我的教學任務」
- **「我的學習地圖」連結只有被指派過學習項目的人才看得到**，沒被指派的同仁畫面上
  不會出現這個選項，避免選單太多雜訊
- **店經理指派教學時可以設定期限**，`learning-assign.html` 每項教學指派旁邊多了一個
  日期欄位
- 對應 API：`GET /api/training/has-assignments`（判斷要不要顯示學習地圖連結）
- 共用元件：`homepage-reminders.js`，跟自評表提醒是同一種設計模式，放在打卡頁/任務頁
  即可自動運作

**部署前要跑 SQL**：執行 `schema_v25_teaching_due.sql`。

## 照片改存到 Supabase Storage（不再塞進資料庫）

原本客訴登記、缺失回報、個人清潔配額拍照，照片都是轉成一長串文字（base64）直接存進
資料庫欄位，這樣資料庫會越塞越大、越來越慢。現在改成：**照片存到 Supabase Storage，
資料庫裡只留一個短短的網址**。

**同仁操作方式完全不變**（一樣是選照片、送出），差別在後端收到照片後怎麼處理。

**部署前，你需要先在 Supabase 手動做一件事（只需要做一次）：**

1. 打開 Supabase 專案 → 左側選單 **Storage**
2. 點 **New bucket**
3. 名稱輸入 `photos`（一定要是這個名字，程式碼裡是寫死的）
4. **Public bucket** 打開（設成公開），這樣照片網址才能直接顯示，不用額外簽發權限
5. 建立完成

沒有先建這個 bucket 的話，上傳照片會失敗**回退**成原本存 base64 的方式（不會噴錯、
不會弄丟照片，只是沒達到省空間的效果），所以務必先做這一步再部署新程式碼。

**這次改的檔案**：新增 `src/lib/uploadPhoto.js`（共用工具），修改
`src/routes/issues.js`、`src/routes/roomCleanings.js`、`src/routes/staffCleaning.js`
這三個檔案，把原本直接存 `photo_url` 的地方都改成先呼叫這個工具轉存。

## 同仁登入密碼制度

原本登入只要輸入登入代碼（例如員編）就能進去，只要知道對方的代碼就能冒用身分。現在
改成**登入代碼＋密碼**兩段式：

- **第一次登入**（`password_hash` 還是空的同仁）：輸入登入代碼後，系統會自動跳出「請設定
  密碼」畫面，設定完直接登入，之後每次都要輸入這組密碼
- **已經設定過密碼的同仁**：輸入登入代碼後，跳出密碼輸入畫面，密碼對了才能登入
- 密碼**沒有格式限制**（數字英文都可以，長度不限），對照顧同仁使用習慣

**技術細節**：密碼不是明文存的，是用 bcrypt 加密雜湊後存進 `password_hash` 欄位，資料庫
裡看不到原始密碼（就算資料庫外洩，密碼本身也不會被直接看到）。

**部署前要做兩件事**：
1. 執行 `schema_v29_password.sql`
2. `package.json` 有新增 `bcryptjs` 這個套件依賴，**上傳新版 `package.json` 之後
   Railway 會自動重新安裝套件**，不用你自己額外操作

**重要提醒**：這個密碼制度解決的是「同事間互相冒用登入代碼」的風險，**不是**後端 API
層級的身分驗證（那個是更大的工程，之前討論過，還沒做）。

## 密碼管理（重設密碼）

密碼本身**沒有辦法查看**（加密後不可逆），但可以重設：

- `manage-passwords.html`（總公司總覽頁、店經理儀表板都有連結）
  - **總公司**登入：看得到全部館別同仁的密碼設定狀態
  - **店經理**登入：只看得到自己館別的同仁
  - 每位同仁顯示「已設定密碼」或「尚未設定」，已設定的可以按「重設」
  - 重設之後，該同仁密碼欄位會被清空，**下次登入會自動跳回「第一次登入設定密碼」的
    畫面**，不需要知道原本密碼是什麼
- 對應 API：`GET /api/staff/password-status`、`POST /api/staff/:id/reset-password`

> 2026-09 起這一頁已經改成完整的「帳號管理」，見下一節。

## 帳號管理（新增／修改／停用帳號、設定密碼）

以前改帳號、改密碼都要進 Supabase 資料表手動改，現在總公司和店經理可以直接在系統裡處理。
頁面沿用 `manage-passwords.html` 這個檔名，所以原本連到「密碼管理」的連結都不會壞。

- 入口：總公司側邊選單「帳號管理」、店經理側邊選單「帳號 → 帳號管理」、總公司總覽頁上方連結
- 可以做的事：
  - 搜尋姓名／登入代碼、依館別篩選（總公司）、顯示已停用的帳號
  - **新增帳號**：姓名、登入代碼、館別、職務、是否兼職、初始密碼（選填）
  - **編輯帳號**：姓名、登入代碼、館別、職務、是否兼職
  - **設定密碼**：直接給一組新密碼；主管也可以用這個改自己的密碼
  - **清空密碼**：對方下次登入時輸入的密碼會直接變成新密碼（原本「重設」的行為）
  - **停用／重新啟用**：停用後不能登入，但自評表等所有紀錄都保留
  - **永久刪除**：只有「已停用」而且「從來沒設定過密碼」的帳號才能刪（通常是建錯的帳號）

**密碼沒辦法查看**：資料庫只存加密後的雜湊值，任何人（包含總公司）都看不到原本的密碼，
API 也不會回傳雜湊值。

**權限**（後端依資料庫裡的真實職務判斷，不是只靠畫面）：

| 誰 | 可以管理 |
|---|---|
| 總公司 | 所有館別、所有職務的帳號 |
| 店經理 | 只有自己館別的**房務、客務**帳號；自己的帳號只能改密碼，不能動其他店經理或總公司帳號 |
| 其他人 | 不能使用 |

**防呆**：
- 登入代碼不能重複（含已停用的帳號），因為登入時重複的代碼會讓兩個人都登不進去
- 不能停用、刪除自己的帳號，也不能改自己的職務
- 不能停用或改掉「最後一個啟用中的總公司帳號」，否則就沒有人能進後台
- 永久刪除前會檢查 `staff_schedule`（排班）、`staff_learning_progress`（培訓進度）、
  `assessment_stage_evaluations`（階段考核）這三張表——它們的外鍵是 `on delete cascade`，
  有資料的話刪帳號會一起被清掉，所以有資料就不准刪。其他指向 staff 的表由資料庫外鍵擋下
  （注意：最初建表的 `schema_v2.sql` 不在 repo 裡，那幾張表的外鍵設定沒辦法從程式碼確認）

- 對應 API（全部要帶 `?actor_id=目前登入的同仁 id`）：
  `GET /api/accounts/meta`、`GET /api/accounts`、`POST /api/accounts`、`PUT /api/accounts/:id`、
  `POST /api/accounts/:id/password`、`POST /api/accounts/:id/reset-password`、
  `POST /api/accounts/:id/deactivate`、`POST /api/accounts/:id/activate`、`DELETE /api/accounts/:id`

**⚠ 已知限制：這組 API 沒有真正的登入驗證。** 後端會用 `actor_id` 去查這個人真實的職務與館別再判斷權限，
可以擋住畫面上的越權操作，但擋不住知道 API 的人自己偽造 `actor_id`（例如冒充總公司去新增帳號）。
舊的 `POST /api/staff/:id/reset-password` 也一樣不用登入就能呼叫。要真正防堵需要另外加上登入驗證，
2026-09-13 討論後決定先只做畫面，之後再處理。

**不用跑 SQL**：用的都是 `staff` 表既有的欄位。

## 櫃檯加給考核筆試

依你提供的考卷建置（是非15題＋選擇15題＋問答8題，共38題，**台中館、台東2館共用同一份
題庫**）。因為選擇題需要標準答案才能自動改考卷、問答題更是要人工評分，所以這不是線上
自動測驗，是「題庫展示＋人工記錄結果」：

- 在「指派培訓」頁（`learning-assign.html`），選職務類別選**客務**，會出現「櫃檯加給考核
  筆試」區塊
- 點「顯示／隱藏完整題目」可以看到整份題庫（列印或線上出題都可以參考）
- 考完之後，把日期、分數、合格與否、考核者記錄下方，會顯示歷史紀錄
- 對應 API：`GET /api/written-exam/questions`、`GET/POST /api/written-exam/results`

**注意**：問答題第1題原始考卷有附一張2024年4月的月曆圖片，數位系統沒辦法呈現圖片，
出題時請自行附上當月月曆。

**部署前要跑兩份 SQL，依序執行**：`schema_v30_written_exam.sql` → `schema_v30_seed.sql`。

## 功能開關（總公司後台）

系統功能一直在長，但不是每一項都準備好正式上線。這個功能讓**總公司**可以整組開啟／隱藏
功能，例如目前的階段：**只開放每月自評表，其他功能全部先關起來**。

- `feature-toggles.html`（總公司側邊選單「帳號管理 → 功能開關」，總公司總覽頁上方也有連結）
  - 上方兩顆快速按鈕：**只開放自評表**（一鍵關閉其餘全部）、**全部開啟**（回到完整狀態）
  - 下方是逐項開關，依房務／客務／共用／主管分組，可以只關掉其中幾項
  - 設定是**全公司共用**（不分館別），存好之後同仁重新整理頁面就生效
- 對應 API：`GET /api/features`、`PUT /api/features/:key`、`POST /api/features/bulk`、
  `POST /api/features/preset/self-eval-only`、`POST /api/features/preset/enable-all`

**關掉一個功能之後會發生三件事**（三層都擋，不是只把選單藏起來）：

1. **選單不顯示**——`sidebar.js` 會過濾掉那些頁面，整個分類都空了連分類按鈕也會消失
2. **直接打網址沒用**——進到被關閉的頁面會顯示提示並自動導回自評表；登入時如果原本
   該去的頁面（例如房務的 `checklist.html`）被關了，也會直接導到自評表
3. **API 回 403**——`src/lib/featureGuard.js` 掛在所有 `/api` 之前，就算有人自己打 API
   也會被擋下來

**兩個永遠關不掉的保留項目**（避免整個系統被鎖死到沒人能開回來）：

- `self_eval` 每月自評表（含主管審閱、總公司題目管理與異常彙總）
- `system_admin` 總公司儀表板、功能開關、密碼管理

登入、同仁清單、以及登入時查當日班別用的 `GET /api/schedule/today` 這幾支也一律放行，
不然排班表關掉之後連登入導頁都會壞掉。

**功能清單寫在哪**：`src/lib/featureCatalog.js`（一個功能包含哪些網頁、哪些 API 前綴）。
資料庫的 `feature_toggles` 表只存「哪個功能被關掉」，所以之後新增功能只要在 catalog 加一筆
就好，不用再改資料庫；catalog 有但資料表沒有的項目一律視為**開啟**。

**容錯**：`feature_toggles` 表還沒建立、或 Supabase 查詢失敗時，一律當作「全部開啟」，
不會因為開關系統自己出問題就讓整個系統不能用（所以先上程式碼、之後再跑 SQL 也不會壞）。

**部署前多一步**：先到 Supabase SQL Editor 執行 `schema_v37_feature_toggles.sql`
（預設全部開啟，跟現在行為完全一致，跑完不會有任何改變，要關再到後台按）。

## 測試環境（staging schema）

以前沒有地方可以安全試改動：只有一個 Supabase 專案、免費方案的 2 個專案名額也用完了，
所以每次改完都是直接在正式環境驗證。

現在改成**在同一個 Supabase 專案裡開第二個 schema**：

```
norden-daily-ops（同一個 Supabase 專案，同一組金鑰）
├── public   schema  ← 正式 Railway service（真實資料）
└── staging  schema  ← 測試 Railway service（假資料，隨便玩）
```

程式碼裡所有資料存取都是 `supabase.from('表名')`，沒有任何地方寫死 schema，
所以 `src/supabaseClient.js` 指定預設 schema，整個系統就會整組切換：

```js
const DB_SCHEMA = process.env.SUPABASE_DB_SCHEMA || 'public';
```

**正式環境不要設這個環境變數**，預設就是 `public`，行為跟以前完全一樣。
測試用的 Railway service 設 `SUPABASE_DB_SCHEMA=staging` 就好。

打開 `/api/health` 可以確認目前連的是哪一邊：

```json
{ "status": "ok", "service": "norden-daily-ops-backend", "db_schema": "staging" }
```

### 建立測試環境

1. Supabase SQL Editor 執行 `staging_01_setup.sql`
   （複製 public 的**結構**到 staging，不複製任何資料；含重建外鍵）
2. Supabase SQL Editor 執行 `staging_02_seed.sql`
   （複製館別／職務／房號／各種題目範本，並建立 4 個假帳號）
3. Supabase → **Settings → API → Exposed schemas** 把 `staging` 加進去
   （沒做這步 PostgREST 讀不到 staging，API 會回錯誤）
4. Railway 新增第二個 service，指向同一個 repo、**Branch 選要測的 branch**，
   環境變數除了原本兩個之外多加 `SUPABASE_DB_SCHEMA=staging`

測試帳號：`T-HQ`（總公司）、`T-MGR`（店經理）、`T-HK`（房務）、`T-FD`（客務）。
密碼欄位是空的，第一次登入輸入什麼就會變成密碼。

### 以後有新的 SQL 要怎麼跑

**一份新的 `schema_v*.sql` 要跑兩次**：一次進 staging（先測），一次進 public（正式）。

先在 staging 測：把這一行加在 SQL 的**最前面**，其餘內容原封不動貼在後面，一起執行——

```sql
set search_path to staging, public;
```

`search_path` 的第一個是 staging，所以 `create table xxx` 會建在 staging；
後面接 public 是為了萬一 SQL 裡有查到既有資料的地方也不會出錯。

測沒問題之後，**把那一行刪掉**，一模一樣的 SQL 再跑一次，就會進 public（正式）。

> ⚠ 兩邊結構不同步的話測試結果就不準了，這是這個做法最主要的維護成本。
> 忘記哪些表沒同步時，重跑 `staging_01_setup.sql` 就會自動補上缺的表、跳過已經有的。

### 兩個要記得的限制

- **每份新 SQL 都要記得跑兩次**（見上一段），漏跑 staging 就會測到假的結果。
- **Storage 的 `photos` bucket 是共用的**，測試上傳的缺失照片會跟正式照片放在同一個
  bucket。不影響功能，只是會多幾張用不到的圖。

### 為什麼不是用 Supabase Branching

Supabase Branching 是照著 repo 裡 `supabase/migrations/` 的檔案**重建一個空資料庫**，
而這個專案的 SQL 是歷史流水帳、不是 migrations 格式；更關鍵的是**最初建表的
`schema_v2.sql` 根本不在 repo 裡**，所以「重跑 SQL 檔重建資料庫」這條路本來就走不通。
從現有的 public 動態複製結構才是可靠的做法。
（Branching 另外還需要 Pro 方案。）

## 已知限制 / 下一步

- **缺失照片**是用瀏覽器直接轉成 base64 存進資料庫的文字欄位，量大或照片解析度高時
  資料庫會變得肥大。之後建議改成上傳到 Supabase Storage，資料庫只存網址連結。
- 主管端電腦儀表板還沒做（今日獎金總表、缺失趨勢、樓主細清進度）。
- 目前任何「房務」類角色都能看到房號分配跟細清排程連結，還沒有依「當天是否為
  小隊長／樓主」做權限限制（你確認過小隊長不用權限控管，先維持人工自律）。
- 排班表的「特」「中」「管」「PT1」「PT2」目前沒有對應的任務頁面，登入時會退回用固定
  職務判斷；如果之後想幫這些班別也做專屬頁面，可以再討論。
- 教育訓練系統（新人學習地圖、教練制度、四階段考核）還沒開始建，設計方向已在
  對話中討論過，包含 learning_paths / learning_units / staff_learning_progress /
  coach_assignments / assessment_stages / performance_reviews 這幾張表。
